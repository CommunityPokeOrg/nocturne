...
- **Android GPS notifications:** `NavNotificationBridge` uses the same effective `showNativeNotifications` gate as mirrored phone notifications in both skins. Disabling Phone Notifications clears active guidance and its timer, suppresses subsequent `nav.update` events, and resets maneuver deduplication. Re-enabling permits future updates without replaying suppressed guidance.
